...
### 4a. 沒有辨識 Scope Multiplier

過去案例多為單一科目；本次跨國語、數學、生活，涵蓋接近半學期／整冊的 28 個單元。Agent 卻沿用單科任務的節奏、狀態報告與完成定義，沒有切換成 portfolio mode。

**預防：** Phase 0 先記錄 subject count、unit count、semester range、artifact count、publisher count。跨科或半學期規模自動進入 portfolio mode：每科獨立 Golden、artifact ledger、QA ledger 與 promotion wave。

### 4b. 沒有辨識 Continuity Break

本案不是連續 session。Repo、production、NotebookLM、YouTube、本機 assets 與 Obsidian 可能停在不同 checkpoint；Agent 卻把過去陳述直接當成目前 evidence，等於在未驗證狀態下前進一階。

**預防：** resumed run 必須做 continuity audit，檢查 HEAD／worktree、production、artifact manifest、雲端產物、帳號與路徑。先退回上一個已驗證 gate，重新確認後才恢復進度。
...
- `scopeClass` 是 unit、subject 還是 portfolio？是否已按科目分波？
- `continuityStatus` 是 continuous 還是 resumed？若 resumed，是否已退回上一個 gate 做 drift audit？
...
本案的 resumed portfolio 起點見 `docs/reuse-map-2026-07-11.md`。該文件是新建置前的 evidence snapshot；重新 resumed 時要建立新的 dated map，不得把舊 map 當成目前狀態。
