...
開始下載前，先查 `data/source-registry/`；每個下載、轉檔或未下載候選都要記入 `data/source-acquisition-log.json`，規格見 `docs/source-acquisition.md`。原始檔仍放 ignored `source/`，但 provenance log 必須納入 Git。**檔名、URL 或搜尋摘要有「115」不構成年份核對：必須核對 PDF／文件內文的年度與版本；不一致即記 rejection，不抽取任何內容。** YouTube／教師網站預設是 Tier C 候選：先查版本、授權與教師 review，不能直接當課文事實或 NotebookLM source。
