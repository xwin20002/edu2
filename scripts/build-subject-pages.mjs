...
const chineseExtensionHtml = unit => {
  ...
  const summary = unit.verticalSummary ? `<div class="section-title"><span>📖</span> 直式導讀</div><section class="card"><p class="vert-summary">${esc(unit.verticalSummary)}</p></section>` : "";
...
const chineseSourceWordBankHtml = unit => {
  ...
  if (!intake) return "";
  const terms = [...intake.sourceTerms["生字"].slice(0, 8), ...intake.sourceTerms["語詞"].slice(0, 4)];
  return `<div class="section-title"><span>📚</span> 已核對詞彙入口</div><section class="card"><p>已核對課名與教育雲公開詞彙；本站學習焦點與任務是原創教學草稿，課文脈絡仍須搭配合法課本或教師核對稿確認。</p><div class="topic-flow">${terms.map(term => `<span class="topic-pill">${esc(term)}</span>`).join("")}</div><p><a href="${esc(intake.sourceUrl)}" target="_blank" rel="noreferrer">在教育雲查看本課完整詞彙表 →</a></p></section>`;
};
...
for (const subject of data.subjects) {
  ...
  for (const [index, unit] of subject.units.entries()) {
    ...
    const misconception = subject.id === "chinese" ? "只認字就等於讀懂；其實還要依線索理解事件、情緒與畫面。" : subject.id === "math" ? "只追求算得快；其實要能用圖像、操作或生活語言說明方法。" : "只是在玩遊戲；其實每次實作都要觀察、記錄、比較並分享發現。";
