...
for (const subject of data.subjects) {
  ...
  for (const [index, unit] of subject.units.entries()) {
    ...
    const pageWithSourceLayer = sourceLayer
      ? page.replace(`</section><div class="section-title"><span>🎯</span> 學習目標</div>`, `</section>${sourceLayer}<div class="section-title"><span>🎯</span> 學習目標</div>`)
      : page;
    const pageWithChineseSource = subject.id === "chinese"
      ...
      : pageWithSourceLayer.replace(practicalMarker, `${mathExtension}${practicalMarker}`);
